...
// ---------------------------------------------------------------------
// Fork-only: openFileInNewTab. Inline-previews a file in a new browser
// tab via blob URL + MIME hint, falling back to download for non-inline
// extensions or popup-blocked windows. Used by the directory tree's
// per-row action bar; upstream does not surface this affordance.
// ---------------------------------------------------------------------
...
function getInlineExt(name: string): string {
...
async function fetchInlineBlob(workerId: string, filePath: string, mimeType: string): Promise<Blob> {
  const parts: BlobPart[] = []
  for await (const { chunk } of streamFileChunks(workerId, filePath))
    parts.push(chunk)
  return new Blob(parts, { type: mimeType })
}

export async function openFileInNewTab(workerId: string, filePath: string, flavor: PathFlavor): Promise<void> {
  const name = basename(filePath, flavor)
  const mime = INLINE_EXT_MIME[getInlineExt(name)]
  if (mime === undefined) {
    await downloadFileFromWorker(workerId, filePath, flavor)
    return
  }
  try {
    const blob = await fetchInlineBlob(workerId, filePath, mime)
