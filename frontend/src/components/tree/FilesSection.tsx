...
import { localStorageGet, localStorageSet, PREFIX_FILES_NAME_WRAP, PREFIX_FILES_SHOW_HIDDEN } from '~/lib/browserStorage'
...
export const FilesSection: Component<FilesSectionProps> = (props) => {
  ...
  const [wrapName, setWrapName] = createSignal(localStorageGet<boolean>(nameWrapStorageKey()) ?? true)
  ...
  createEffect(on(nameWrapStorageKey, (key) => {
    setWrapName(localStorageGet<boolean>(key) ?? true)
  }, { defer: true }))

  // Persist name-wrap when toggled (skip initial mount).
  createEffect(on(wrapName, (value) => {
    localStorageSet(nameWrapStorageKey(), value)
  }, { defer: true }))
